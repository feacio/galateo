object dlg_elenco_macros: Tdlg_elenco_macros
  Left = 553
  Top = 116
  BorderIcons = [biSystemMenu]
  Caption = 'Elenco macro'
  ClientHeight = 438
  ClientWidth = 235
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -13
  Font.Name = 'System'
  Font.Style = []
  FormStyle = fsStayOnTop
  Menu = menu
  OldCreateOrder = True
  Visible = True
  OnClose = FormClose
  OnCreate = FormCreate
  OnDestroy = FormDestroy
  OnKeyDown = FormKeyDown
  OnResize = FormResize
  PixelsPerInch = 96
  TextHeight = 16
  object split: TSplitter
    Left = 0
    Top = 275
    Width = 235
    Height = 7
    Cursor = crVSplit
    Align = alBottom
    Beveled = True
    ExplicitTop = 276
  end
  object lb: TListBox
    Left = 0
    Top = 0
    Width = 235
    Height = 275
    Align = alClient
    PopupMenu = Popup
    TabOrder = 0
    OnClick = lbClick
    OnDblClick = lbDblClick
  end
  object Panel1: TPanel
    Left = 0
    Top = 391
    Width = 235
    Height = 47
    Align = alBottom
    TabOrder = 1
    object btn_load: TBitBtn
      Left = 5
      Top = 8
      Width = 106
      Height = 33
      Caption = 'Carica'
      Default = True
      Glyph.Data = {
        
        FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF000000}
      Layout = blGlyphRight
      Spacing = 12
      TabOrder = 0
      OnClick = btn_loadClick
    end
    object btn_copy: TBitBtn
      Left = 125
      Top = 8
      Width = 106
      Height = 33
      Caption = 'Copia'
      Glyph.Data = {
        
        9990}
      Layout = blGlyphRight
      Spacing = 12
      TabOrder = 1
      OnClick = btn_copyClick
    end
  end
  object str_definizione: TMemo
    Left = 0
    Top = 282
    Width = 235
    Height = 109
    Align = alBottom
    Color = 8454143
    ReadOnly = True
    ScrollBars = ssVertical
    TabOrder = 2
  end
  object menu: TMainMenu
    Left = 28
    Top = 40
    object File1: TMenuItem
      Caption = '&File'
      object itm_load: TMenuItem
        Caption = 'Carica macro'
        OnClick = itm_loadClick
      end
      object itm_primo_piano: TMenuItem
        Caption = '&Sempre in primo piano'
        ShortCut = 117
        OnClick = itm_primo_pianoClick
      end
      object N1: TMenuItem
        Caption = '-'
      end
      object itm_close: TMenuItem
        Caption = '&Chiudi'
        ShortCut = 32883
        OnClick = itm_closeClick
      end
    end
    object itm_modifica: TMenuItem
      Caption = '&Modifica'
      object itm_copy: TMenuItem
        Caption = '&Copia'
        ShortCut = 16451
        OnClick = itm_copyClick
      end
    end
  end
  object Popup: TPopupMenu
    Left = 84
    Top = 124
    object itp_load: TMenuItem
      Caption = 'Carica macro'
      OnClick = itp_loadClick
    end
    object itp_copy: TMenuItem
      Caption = 'Copia definizione'
      OnClick = itp_copyClick
    end
  end
end
