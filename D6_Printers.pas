...
{*$L-,D-}		// in generale qui dentro non serve il debug
...
uses Windows, WinSpool, SysUtils, Classes, Graphics, Forms, Consts,
	dich;
...
uses galateo_debug, FAssert, FErrMsg
	*,pages {$endif};

var i_default_printer_index : smallint;
...
var prn : ^PrnRec;
begin
//	PrnString(PrnRec(F.UserData), PChar(F.BufPtr), F.BufPos);
	prn := @f.UserData;
	PrnString(prn^, PChar(F.BufPtr), F.BufPos);
	F.BufPos := 0;
	Result := 0
end;
...
var ptrprn : ^PrnRec;
begin
	with F{, PrnRec(F.UserData)} do begin
...
//			Cur.X := 0;Cur.Y := 0;
//			Finish.X := Printer.PageWidth;Finish.Y := Printer.PageHeight;Height := 0
			ptrprn := @UserData;
			ptrprn.Cur.X := 0;ptrprn.Cur.Y := 0;
			ptrprn.Finish.X := Printer.PageWidth;ptrprn.Finish.Y := Printer.PageHeight;
			ptrprn.Height := 0
		end;
		Result := 0
	end
...
//	if {(i_phisical_10mm_height = 0) AND} (devmode.dmPaperLength <> 0) then begin
	if (i_phisical_10mm_height = 0) AND (devmode.dmPaperLength <> 0) then begin
		...
		*runtime_debug('paperLenght = ' + inttostr(i_phisical_10mm_height) + ' mm','SetPrinterCapabilities()',RD_DEBUG_ACCESSORIO_01) {$endif}
	end;
...
//	if {(i_phisical_10mm_width = 0) AND} (devmode.dmPaperWidth <> 0) then begin
	if (i_phisical_10mm_width = 0) AND (devmode.dmPaperWidth <> 0) then begin
		...
		*runtime_debug('paperWidth = ' + inttostr(i_phisical_10mm_width) + ' mm','SetPrinterCapabilities()',RD_DEBUG_ACCESSORIO_01) {$endif}
	end;
...
//			DeviceMode := GlobalAlloc(GHND, DocumentProperties(0, FPrinterHandle, ADevice, StubDevMode, StubDevMode, 0));
			{	fino al 2019-05-xx ha SEMPRE funzionato la singola riga qui sopra; ora pare non funzioni più; vedi anche
				https://stackoverflow.com/questions/37382720/documentproperties-fails-in-xe6-works-in-delphi-7
				comunque non sembra essere un problema del tutto invalidante, atteso che il problema si manifesta solo all'avvio e solo nella DLL,
				mentre nel prosieguo tutto SEMBRA funzionare bene }
			lo := DocumentProperties(0, FPrinterHandle, ADevice, StubDevMode, StubDevMode, 0);
			writeln_system_debug(121, MBOX_RUNTIME_DEBUG_CAPTION, 'DocumentProperties=' + inttostr(lo));
{			if (lo = -1) then begin
				GetDriverInfo(FDriverHandle);	** resetta il system e fa rifunzionare tutto
				lo := DocumentProperties(0, FPrinterHandle, ADevice, StubDevMode, StubDevMode, 0);
				writeln_system_debug(124, MBOX_RUNTIME_DEBUG_CAPTION, 'DocumentProperties=' + inttostr(lo));
			end;
			writeln_system_debug(125, MBOX_RUNTIME_DEBUG_CAPTION); }
			if (lo = -1) then lo := 10000;	// su Win7 dava una dimensione pari a 1689, 10k è cautelativo
			DeviceMode := GlobalAlloc(GHND, lo);
			writeln_system_debug(130, MBOX_RUNTIME_DEBUG_CAPTION);
			if (DeviceMode <> 0) then begin
				writeln_system_debug(140, MBOX_RUNTIME_DEBUG_CAPTION);
				DevMode := GlobalLock(DeviceMode);
				writeln_system_debug(150, MBOX_RUNTIME_DEBUG_CAPTION);
				lo := DocumentProperties(0, FPrinterHandle, ADevice, DevMode^, DevMode^, DM_OUT_BUFFER);
				if (lo < 0) then begin
		...
		// start federico
			...
			lo := DocumentProperties(0, FPrinterHandle, ADevice, DevMode^, DevMode^, DM_MODIFY OR DM_OUT_BUFFER);
			if (lo < 0) then begin
...
begin
	...
	*runtime_debug('GetPageHeight = ' + inttostr(result),'GetPageHeight',RD_DEBUG_ACCESSORIO_01) {$endif}
end;
...
begin
	...
	*runtime_debug('GetPageWidth = ' + inttostr(result),'GetPageWidth',RD_DEBUG_ACCESSORIO_01) {$endif}
end;

function TPrinter.GetPrinterIndex : Integer;
begin
	bo_nessuna_stampante_installata := (printers.Count = 0);
	if (FPrinterIndex = -1) AND NOT bo_nessuna_stampante_installata then SetToDefaultPrinter;
...
//			then SetPrinter(LPSTR(str_devicename), '', '', 0)
			then SetPrinter(PChar(str_devicename), '', '', 0)
	end;
	FreeFonts;
	SetState(psNoHandle)
end;
...
//				lo := DocumentProperties(0, FPrinterHandle, LPSTR(str_setprinted_device), DevMode^, DevMode^, DM_OUT_BUFFER);
				lo := DocumentProperties(0, FPrinterHandle, PChar(str_setprinted_device), DevMode^, DevMode^, DM_OUT_BUFFER);
				if (lo < 0) then begin
					reset_devicemode;
					abort
				end
...
//		lo := DocumentProperties(wh, FPrinterHandle, LPSTR(str_setprinted_device), DevMode^, DevMode^, DM_IN_BUFFER OR DM_PROMPT OR DM_OUT_BUFFER);
		lo := DocumentProperties(wh, FPrinterHandle, PChar(str_setprinted_device), DevMode^, DevMode^, DM_IN_BUFFER OR DM_PROMPT OR DM_OUT_BUFFER);
...
initialization
	galateo_initialization_debug('xprinters');
	init_printer
finalization
	galateo_finalization_debug('xprinters');
//	Printer.Free;
	printer := NIL
end.
